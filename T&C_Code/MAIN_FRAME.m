...
for i=2:NN
    ...
    if (Datam(i,4)==1)
        ...
        for cc=1:length(Ccrown)
            ...
            Nuptake_H(j,cc)= 0.0;
            Puptake_H(j,cc)= 0.0;
            Kuptake_H(j,cc)= 0.0; %% [gK/m^2 day]
            %%%%
            Nuptake_L(j,cc)= 0.0;  %% [gN/m^2 day]
            Puptake_L(j,cc)= 0.0;
            Kuptake_L(j,cc)= 0.0;
            %%%%
            NavlI(j,:)=[0 0 0];
    ...
    [V(i,:),Vice(i,:),O(i,:),Oice(i,:),ZWT(i),OF(i),OS(i),OH(i,:),OL(i,:),Psi_s_H(i,:),Psi_s_L(i,:),Rd(i),Qi_out(i,:),WTR(i,:),...
        ...
        In_max_urb,In_max_rock,K_usle,tau_sno(i-1),Slo_top,Slo_pot,Asur,Ared,aTop,EK(i-1),q_runon(i),Qi_in(i,:),...
        pow_dis,a_dis,...
...
CkC_ALL = sum(Ccrown.*CkC_H)+sum(Ccrown.*CkC_L);
...
clear Se_bio Psi_bio Tdp_bio VSUM VTSUM  
